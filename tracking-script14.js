(function () {
  ...
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script14'));
  ...
  function sendToAllPlatforms(eventName, clickedText = '') {
    ...
    if (
      ...
      CONFIG.googleAdsId &&
      conversionId &&
      conversionId.trim() !== '' // Checks if conversionId is neither undefined nor empty
    ) {
